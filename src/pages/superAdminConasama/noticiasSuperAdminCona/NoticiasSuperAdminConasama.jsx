...
import { getNoticiasApi, deleteNoticiaApi } from '../../../api/conasama/noticias';
import { DetalleNoticia } from '../../../components/adminconasama/noticias/DetalleNoticia';
import { RegisterNoticiaForm } from '../../../components/adminconasama/noticias/RegisterNoticiaForm';

export function NoticiasSuperAdminConasama() {
    ...
    const [errorMsg, setErrorMsg] = useState(""); // Estado extra por si falla al eliminar
    ...
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
                const response = await getNoticiasApi(auth.token);
    ...
    const confirmDelete = async (idNoticia) => {
        try {
            setErrorMsg("");
            await deleteNoticiaApi(auth.token, idNoticia);
            onReload();
            closeModal(); // Cerramos el modal tras eliminar exitosamente
        } catch (error) {
    ...
    {noticias?.map((noticia, index) => (
        <Col key={index} md={4} className="mb-4">
                    ...
                    {noticia.imagen ? (
                        <Card.Img 
                            ...
                            style={{ 
                                height: '220px', 
                                width: '100%', 
                                objectFit: 'cover', // Centra y recorta para llenar el espacio. Si quieres verla toda cambia a 'contain'
            ...
            <ModalBasic
                ...
                size="md" // Usamos tamaño mediano ('md') o pequeño para alertas en lugar de 'lg'
            />
        </Container>
    );
}