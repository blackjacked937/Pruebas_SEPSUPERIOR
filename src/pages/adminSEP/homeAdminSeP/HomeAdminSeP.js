...
export function HomeAdminSeP() {
    ...
    return (
        <div style={{ backgroundColor: '#F4F6F9', minHeight: '100vh', paddingBottom: '40px' }}>
            <SepHeader 
                title={`Panel de Administración: ${nombreSedeActual}`} 
                subtitle="Resumen de indicadores de riesgo para esta región."
            />
            <Container className="container-home-admin-fase-1 mt-2" style={{ display: 'flex', flexDirection: 'column', padding: '0 20px' }}>
                <Row className="w-100">
                    {loading ? (
                        <Col className="text-center py-5">
                            <div className="spinner-border text-success" role="status" style={{ color: '#7DB747' }}></div>
                ...
                {!loading && (
                    <div className="mt-4 p-3 bg-white rounded border w-100 shadow-sm">
