 ...
 */
export async function getPacientesSensiblesSepSuperior(token) {
  try {
    const url = `${BASE_API_SEP_SUPERIOR_V1}/paciente2/pacientes/sencible/`;
    const params = {
      headers: {
 ...
 */
export async function marcarAtencionEspecialSepSuperior(
  idPaciente,
  data,
  token,
) {
  try {
    const url = `${BASE_API_SEP_SUPERIOR_V1}/paciente2/pacientes/${idPaciente}/`;
    const params = {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
